...
export function Routes() {
  ...
  const openEdit = (r: Route) => {
    setForm(r); setEditing(true); setOpen(true); setErr(''); setMsg('')
  }
  ...
  return (
    <div>
      <div className="section-header">
        ...
        <button className="btn btn-primary btn-sm" onClick={openAdd}>+ Add Route</button>
      </div>
      ...
      {isSuperAdmin && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '.75rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
          ...
          {filterClub && (
            <button className="btn btn-ghost btn-sm" onClick={() => setFilterClub('')}>
              ✕ Clear
            </button>
          )}
        </div>
