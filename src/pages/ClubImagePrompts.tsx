...
export function ClubImagePrompts() {
  ...
  const loadMasters = async () => {
    ...
    // Auto-select if only one master image exists
    if (data && data.length === 1 && !form.image_id) {
      setForm(prev => ({ 
        ...prev, 
  ...
  const extractImageUrl = (response: any): string | null => {
    // 1. Check for standard Chat Completion content (sometimes URL is just text)
    if (response.choices?.[0]?.message?.content) {
      const content = response.choices[0].message.content.trim();
      // Use regex to find a URL if the AI wrapped it in markdown or text
      const urlMatch = content.match(/https?:\/\/[^\s"'>)]+/);
      if (urlMatch) return urlMatch[0];
    }

    // 2. Check for OpenRouter's specific image tool/modality output
    // Flux often returns a 'data' array similar to OpenAI's DALL-E 3
    if (response.data?.[0]?.url) return response.data[0].url;

    // 3. Check for 'images' array (used by some OpenRouter providers)
    if (response.images?.[0]) {
      return typeof response.images[0] === 'string'
        ? response.images[0]
        : response.images[0].url;
    }

    // 4. Check for nested modality content (rare but possible in 2026)
    const nestedImage = response.choices?.[0]?.message?.images?.[0];
    if (nestedImage?.url) return nestedImage.url;

    return null;
  };

  const uploadImageFromUrl = async (url: string, index: number, folder: string = 'clubs'): Promise<string> => {
    let blob: Blob
    ...
    const fileName = `samya-ai-part${index}-${Date.now()}.png`
    ...
    try {
      ...
      for (let i = 0; i < total; i++) {
        try {
            ...
            body: JSON.stringify({
              ...
              messages: [{
                ...
                content: form.image_url
                  ? [
                    { type: "text", text: `${form.image_spec || ''}. ${promptParts[i]}`.trim() },
                    { type: "image_url", image_url: { url: form.image_url } }
                  ]
                  : `${form.image_spec || ''}. ${promptParts[i]}`.trim()
              }]
            })
          })

          if (!iRes.ok) throw new Error(`Generation failed at part ${i + 1} (HTTP ${iRes.status})`)

          const iData = await iRes.json()
          let imageUrl = extractImageUrl(iData)
          if (!imageUrl) throw new Error(`No image found in AI response for part ${i + 1}`)

          setMsg(`💾 Uploading image ${i + 1} to storage...`)
          let uploadedUrl = await uploadImageFromUrl(imageUrl, i, 'clubs')
          ...
          console.error(`Error in part ${i + 1}:`, itemErr)
          // We don't throw here, just alert the user and continue
          setErr(`Part ${i + 1} failed: ${itemErr.message}. Continuing...`)
          // Pause briefly so user can see the error
          await new Promise(r => setTimeout(r, 2000))
          setErr('')
        }
      }

      setMsg(`✅ Finished processing! Generated ${total} images.`)
    } catch (e: any) {
