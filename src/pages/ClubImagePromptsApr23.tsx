...
export function ClubImagePrompts() {
    ...
    // Helper to extract image URL from OpenRouter response
    ...
    // Helper to upload a remote image URL or data URL to Supabase storage
    ...
    const generateAiImage = async () => {
        ...
        try {
            setMsg(`✨ Generating image from prompt...`)
                ...
                body: JSON.stringify({
                    ...
                    messages: [{
                        ...
                        content: form.image_url
                            ? [
                                { type: "text", text: form.image_prompt || 'Generate image' },
            ...
            if (!iRes.ok) throw new Error(`Image generation failed (${iRes.status})`)
            ...
            const { error: insErr } = await supabase.from('club_images').insert({
                ...
                caption: form.image_type || 'AI Prompt',
                prompt_id: form.prompt_id
            })

            if (insErr) throw insErr
            setMsg('✅ AI Image generated and saved to club_images!')
        } catch (e: any) {
